...
table(tree1.pred)
table(test.data$churn)


tree1.cm <- table(true = test.data$churn, predicted = tree1.pred)
tree1.cm
...
df.ap<-data.frame(time=abcChurn,
                  event=ifelse(abcChurn$churn==1,0,1),
                  group=abcChurn$godine)
...
library(coin)
wilcox_test(abcChurn$churn~abcChurn$pol)
library(coin)
install.packages("coin", repos="http://coin.rstudio.com/", dependencies=TRUE)
library(coin)
install.packages("coin")
library(coin)
?wilcox.test
